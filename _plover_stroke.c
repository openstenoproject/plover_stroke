...
// Py_UNREACHABLE is only avaiable with Python >= 3.7.
...
static Py_UCS4 key_to_letter(PyObject *key, key_side_t *key_side)
{
    int         kind;
    const void *data;
    Py_ssize_t  len;
    ...
    switch ((len = PyUnicode_GET_LENGTH(key)))
    {
    case 1:
        letter1 = PyUnicode_READ(kind, data, 0);
        if (letter1 == '-')
            break;
        *key_side = KEY_SIDE_NONE;
        return letter1;
    case 2:
        letter1 = PyUnicode_READ(kind, data, 0);
        ...
        if (letter1 == '-')
        {
            if (letter2 == '-')
                break;
            *key_side = KEY_SIDE_RIGHT;
            return letter2;
        }
        if (letter2 != '-')
            break;
        *key_side = KEY_SIDE_LEFT;
        return letter1;
    default:
        break;
    }
...
static stroke_uint_t stroke_from_keys(const stroke_helper_t *helper, PyObject *keys_sequence)
{
    ...
    for (Py_ssize_t num_keys = PySequence_Fast_GET_SIZE(keys_sequence); num_keys--; )
    {
        ...
        for (unsigned k = 0; ; ++k)
        {
           if (k == helper->num_keys)
           {
               PyErr_Format(PyExc_ValueError, "invalid key: %R", key);
               return INVALID_STROKE;
           }

           if (key_side != helper->key_side[k])
              continue;

           if (key_letter == helper->key_letter[k])
               ;
           else if (key_letter == helper->key_number[k])
               mask |= helper->number_key_mask;
           else continue;
           mask |= STROKE_1 << k;
           break;
...
static stroke_uint_t stroke_from_steno(const stroke_helper_t *helper, PyObject *steno)
{
    ...
    if (0 == steno_len || steno_len > helper->num_keys)
        goto invalid;
...
static PyObject *stroke_to_keys(const stroke_helper_t *helper, stroke_uint_t mask)
{
    ...
    for (stroke_index = key_index = 0; key_index < helper->num_keys; ++key_index)
    {
        if ((mask & 1))
        {
    ...
    if (NULL == (keys_tuple = PyTuple_New(stroke_index)))
        goto error;
...
static PyObject *stroke_to_str(const stroke_helper_t *helper, stroke_uint_t mask)
{
    ...
    // Show hyphen if: there's at least one right-hand side key and no implicit hyphen keys.
    need_hyphen = (mask & helper->right_keys_mask) && !(mask & helper->implicit_hyphen_mask);

    for (stroke_index = key_index = 0; key_index < helper->num_keys; ++key_index)
    {
        if ((mask & 1))
        {
            if (need_hyphen && key_index >= helper->right_keys_index)
            {
                stroke[stroke_index++] = '-';
                need_hyphen = 0;
            }
...
unsigned stroke_to_sort_key(const stroke_helper_t *helper, stroke_uint_t mask, char *sort_key)
{
    ...
    for (sort_key_index = key_index = 0; ++key_index <= helper->num_keys; )
    {
        if ((mask & 1))
            sort_key[sort_key_index++] = key_index;
        if (!(mask >>= 1))
            break;
    }
...
static PyObject *StrokeHelper_setup(StrokeHelper *self, PyObject *args, PyObject *kwargs)
{
    ...
    helper.num_keys = 0;
    helper.right_keys_index = (unsigned)num_keys;
    helper.implicit_hyphen_mask = 0;
    helper.number_key_mask = 0;
    helper.numbers_mask = 0;
    helper.right_keys_mask = 0;

    for (unsigned n = 0; n < num_keys; ++n)
    {
        key = PySequence_Fast_GET_ITEM(keys_sequence, n);
        if (!PyUnicode_Check(key))
        {
            PyErr_Format(PyExc_ValueError, "invalid `keys`; key %u is not a string: %R", num_keys, key);
            return NULL;
        ...
        key_mask = STROKE_1 << helper.num_keys;

        switch (key_side)
        {
        case KEY_SIDE_LEFT:
            if (helper.right_keys_index != num_keys)
            {
        ...
        case KEY_SIDE_RIGHT:
            ...
            helper.right_keys_mask |= key_mask;
            break;
        }
        ...
        helper.key_side[helper.num_keys] = key_side;
        helper.key_letter[helper.num_keys] = key_letter;
        ...
        helper.key_number[helper.num_keys++] = key_letter;
    }

    if (number_key_letter && !helper.number_key_mask)
    {
        PyErr_SetString(PyExc_ValueError, "invalid `number_key`");
        return NULL;
    }
    ...
    if (number_key_letter && popcount(helper.numbers_mask) != 10)
    {
    ...
    if (implicit_hyphen_keys != Py_None)
    {
        ...
        // Implicit hyphen keys must be a continous block.
    ...
    else
    {
        ...
        for (k = helper.right_keys_index; k && (unique_letters_mask & (STROKE_1 << --k)); )
            ;
        for (l = helper.right_keys_index; l < num_keys && (unique_letters_mask & (STROKE_1 << l)); ++l)
            ;
...
#if DEBUG
    fprintf(stderr, "%u letters\n", helper.num_keys);

    fprintf(stderr, "number key mask:      ");
    for (unsigned n = 0; n < helper.num_keys; ++n)
        fprintf(stderr, "%c", helper.number_key_mask & (STROKE_1 << n) ? '1' : '0');
    fprintf(stderr, "\n");

    fprintf(stderr, "numbers mask:         ");
    for (unsigned n = 0; n < helper.num_keys; ++n)
        fprintf(stderr, "%c", helper.numbers_mask & (STROKE_1 << n) ? '1' : '0');
    fprintf(stderr, "\n");

    fprintf(stderr, "right keys index: %2u  ", helper.right_keys_index);
    for (unsigned n = 0; n < helper.num_keys; ++n)
        fprintf(stderr, "%c", n == helper.right_keys_index ? '1' : '0');
    fprintf(stderr, "\n");
    fprintf(stderr, "right keys mask:      ");
    for (unsigned n = 0; n < helper.num_keys; ++n)
        fprintf(stderr, "%c", helper.right_keys_mask & (STROKE_1 << n) ? '1' : '0');
    fprintf(stderr, "\n");

    fprintf(stderr, "key letters:          ");
    for (unsigned n = 0; n < helper.num_keys; ++n)
        fprintf(stderr, "%c", helper.key_letter[n]);
    fprintf(stderr, "\n");

    fprintf(stderr, "implicit hyphen mask: ");
    for (unsigned n = 0; n < helper.num_keys; ++n)
        fprintf(stderr, "%c", helper.implicit_hyphen_mask & (STROKE_1 << n) ? '1' : '0');
    fprintf(stderr, "\n");

    fprintf(stderr, "key numbers:          ");
    for (unsigned n = 0; n < helper.num_keys; ++n)
        fprintf(stderr, "%c", helper.key_number[n]);
    fprintf(stderr, "\n");
#endif
...
static PyObject *StrokeHelper_get_keys(const StrokeHelper *self, void *Py_UNUSED(closure))
{
    ...
    for (unsigned n = 0; n < self->helper.num_keys; ++n)
    {
        key = key_str(&self->helper, n);
        if (key == NULL)
        {
            Py_DECREF(keys_tuple);
            return NULL;
        }
        PyTuple_SET_ITEM(keys_tuple, n, key);
    }
...
static PyObject *StrokeHelper_normalize_stroke(const StrokeHelper *self, PyObject *stroke)
{
    ...
    if (0 == stroke_len || stroke_len > self->helper.num_keys)
        goto invalid;
...
static PyObject *StrokeHelper_normalize_steno(const StrokeHelper *self, PyObject *steno)
{
    ...
    strokes = PyMem_Malloc((max_strokes = steno_len / 2 + 1) * sizeof (*strokes));
    if (NULL == strokes)
    {
        PyErr_NoMemory();
        goto end;
    ...
    for (num_strokes = steno_index = 0; steno_index < steno_len; ++steno_index)
    {
        stroke_len = 0;
        while ('/' != (stroke_ucs4[stroke_len] = PyUnicode_READ(steno_kind, steno_data, steno_index))
               && ++stroke_len <= self->helper.num_keys && ++steno_index < steno_len)
            ;
        if ('/' == stroke_ucs4[0])
        {
            if (num_strokes)
    ...
    while (num_strokes--)
        PyTuple_SET_ITEM(result, num_strokes, strokes[num_strokes]);
...
error:
    while (num_strokes--)
        Py_XDECREF(strokes[num_strokes]);
end:
    if (NULL != strokes)
        PyMem_Free(strokes);
    return result;
...
static PyObject *StrokeHelper_steno_to_sort_key(const StrokeHelper *self, PyObject *steno)
{
    ...
    sort_key = PyMem_Malloc(steno_len * sizeof (*sort_key));
    if (NULL == sort_key)
    {
        PyErr_NoMemory();
        goto end;
    ...
    for (sort_key_index = steno_index = 0; steno_index < steno_len; ++steno_index)
    {
        stroke_len = 0;
        while ('/' != (stroke_ucs4[stroke_len] = PyUnicode_READ(steno_kind, steno_data, steno_index))
               && ++stroke_len <= self->helper.num_keys && ++steno_index < steno_len)
        ...
        {
            if (sort_key_index && sort_key[sort_key_index - 1] != 0)
                sort_key[sort_key_index++] = 0;
...
static PyObject *StrokeHelper_stroke_from_keys(const StrokeHelper *self, PyObject *keys_sequence)
{
    ...
    for (Py_ssize_t num_keys = PySequence_Fast_GET_SIZE(keys_sequence); num_keys--; )
    {
        ...
        for (unsigned k = 0; ; ++k)
        {
           if (k == self->helper.num_keys)
           {
               PyErr_Format(PyExc_ValueError, "invalid key: %R", key);
               return NULL;
           }

           if (key_side != self->helper.key_side[k])
              continue;

           if (key_letter == self->helper.key_letter[k])
               ;
           else if (key_letter == self->helper.key_number[k])
               mask |= self->helper.number_key_mask;
           else continue;
           mask |= STROKE_1 << k;
           break;
        }
...
static PyMemberDef StrokeHelper_members[] =
{
    ...
    {"right_keys_mask"     , T_STROKE_UINT, offsetof(StrokeHelper, helper.right_keys_mask)     , READONLY, "Right keys mask."},
